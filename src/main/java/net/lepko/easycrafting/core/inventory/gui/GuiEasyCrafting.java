...
public class GuiEasyCrafting extends GuiTabbed implements
	...
	private static final ResourceLocation GUI_TEXTURE = new ResourceLocation(
			Ref.RES_DOMAIN, "textures/gui/easycraftinggui.png");
	...
	public GuiEasyCrafting(InventoryPlayer playerInventory,
			TileEntityEasyCrafting tileInventory) {
	...
	public void initGui() {
		...
		searchField = new GuiTextField(fontRendererObj, guiLeft + 82,
				guiTop + 6, 89, fontRendererObj.FONT_HEIGHT) {
			@Override
			public void mouseClicked(int x, int y, int button) {
				boolean isInsideSearchBox = x >= this.xPosition
						&& x < this.xPosition + this.width
						&& y >= this.yPosition
						&& y < this.yPosition + this.height;
				if (isInsideSearchBox) {
					if (button == 1) {
						// On right click remove text and add focus.
						this.setFocused(true);
						this.setText("");
						updateSearch(true);
					} else if (button == 2) {
						// On middle click toggle focus (also removes text)
						setSearchFocused(!this.isFocused());
					} else {
						// On any other mouse click, focus
						setSearchFocused(true);
						// Super implementation will only change cursor position
						// when using left click
						super.mouseClicked(x, y, button);
					}
				} else {
					// A click outside the text box happened. We don't want to
					// remove the search just in case it's still being used. But
					// we do want to remove focus to use other key bindings,
					// such as nei's recipe key binding or nei's utility key
					// binding.
	...
	public void initTabs() {
		tabGroup.addTab(new TabEasyCrafting(new ItemStack(ModBlocks.table),
				"Available"));
		tabGroup.addTab(new TabEasyCrafting(new ItemStack(Items.compass),
				"Search"));
	}
	...
	public void drawScreen(int mouseX, int mouseY, float time) {
		...
		if (!wasClicking && leftMouseDown && mouseX >= left && mouseY >= top
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		drawTexturedModalRect(guiLeft + 156, guiTop + 17
				+ (int) (currentScrollValue * 73.0F), scrollTextureX, 240, 12,
				16);

		// Search
		int searchTextureX = xSize - 90 - 7;
		drawTexturedModalRect(guiLeft + searchTextureX, guiTop + 4,
				searchTextureX, 256 - 12, 90, 12);
		...
		if (canCraftCache != null && currentTab != 0) {
			...
			for (int k = 0; k < 40 && k + offset < canCraftCache.length; k++) {
				drawSlotBackground(inventorySlots.getSlot(k), canCraftCache[k
						+ offset]);
			}
		}
	...
	protected void keyTyped(char keyChar, int keyCode) {
		if (!searchField.isFocused()) {
			if (KeyBindings.focusSearch.getKeyCode() == keyCode) {
				setSearchFocused(true);
			} else {
				super.keyTyped(keyChar, keyCode);
			}
		} else if (!checkHotbarKeys(keyCode)) {
			if (keyCode == 1) {
				setSearchFocused(false);
			} else if (searchField.textboxKeyTyped(keyChar, keyCode)) {
				updateSearch(true);
			} else {
				super.keyTyped(keyChar, keyCode);
			}
	...
	private void setSearchFocused(boolean focus) {
	...
	// 6 -> double click
	@Override
	protected void handleMouseClick(Slot slot, int slotIndex, int button,
			int action) {
	...
	private void onCraftingSlotClick(Slot slot, int slotIndex, int button,
			int action) {
		Ref.LOGGER.trace("Clicked: " + slot.getClass().getSimpleName() + "@"
				+ slotIndex + ", button=" + button + ", action=" + action
				+ ", stack=" + slot.getStack());
		...
		if (recipeIndex >= 0 && shownRecipes != null
				&& recipeIndex < shownRecipes.size()) {
			WrappedRecipe r = shownRecipes.get(recipeIndex);
			if (StackUtils.areEqualNoSizeNoNBT(r.getOutput(), slotStack)
					&& craftableRecipes != null && craftableRecipes.contains(r)) {
				recipe = r;
			}
		}
		if (recipe == null) {
		...
		if (finalStackSize > 0) {
			...
			PacketHandler.INSTANCE.sendToServer(new MessageEasyCrafting(recipe,
					isRightClick, isShiftClick));

			if (isRightClick) { // Right click; craft until max stack
				int maxTimes = RecipeHelper
						.calculateCraftingMultiplierUntilMaxStack(slotStack,
								heldStack);
				int timesCrafted = RecipeHelper.canCraft(recipe,
						mc.thePlayer.inventory, RecipeManager.getAllRecipes(),
						false, maxTimes, ConfigHandler.MAX_RECURSION);
				if (timesCrafted > 0) {
					finalStack.stackSize = finalStackSize + (timesCrafted - 1)
							* finalStack.stackSize;
				...
				int timesCrafted = RecipeHelper.canCraft(recipe,
						...
						false, maxTimes, ConfigHandler.MAX_RECURSION);
				if (timesCrafted > 0) {
					finalStack.stackSize *= timesCrafted; // ignore
															// finalStackSize;
															// it might contain
															// heldStack size
					InventoryUtils.addItemToInventory(mc.thePlayer.inventory,
							finalStack);
				}
	...
	private void updateSearch(boolean scrollToTop) {
		List<WrappedRecipe> all = currentTab == 0 ? craftableRecipes
		...
		if (!LAST_SEARCH.trim().isEmpty()) {
			for (WrappedRecipe recipe : all) {
				List<String> tips = recipe.getOutput().getTooltip(mc.thePlayer,
	...
	private void setRowOffset(int rowOffset) {
		...
		currentScrollValue = MathHelper.clamp_float(currentRowOffset
				/ (float) maxRowOffset, 0F, 1F);
		setSlots();
	}

	private void setScrollValue(float scrollValue) {
		currentScrollValue = MathHelper.clamp_float(scrollValue, 0F, 1F);
		currentRowOffset = MathHelper.clamp_int((int) (currentScrollValue
				* maxRowOffset + 0.5F), 0, maxRowOffset);
		setSlots();
	}
		...
		if (shownRecipes != null) {
			...
			for (int i = 0; i < 40; i++) {
				if (i + offset >= shownRecipes.size() || i + offset < 0) {
					...
					ItemStack is = recipe.handler.getCraftingResult(recipe,
	...
	public List<String> handleItemTooltip(GuiContainer gui, ItemStack stack,
			int mouseX, int mouseY, List<String> currentTip) {
	...
	protected void renderToolTip(ItemStack stack, int mouseX, int mouseY) {
		if (!drawCustomTooltip(stack, mouseX, mouseY,
				(List<String>) stack.getTooltip(mc.thePlayer,
						mc.gameSettings.advancedItemTooltips))) {
			super.renderToolTip(stack, mouseX, mouseY);
		}
	}

	private boolean drawCustomTooltip(ItemStack stack, int mouseX, int mouseY,
			List<String> currentTip) {
		if (isCtrlKeyDown() && currentTip != null && !currentTip.isEmpty()) {
			for (int j = 0; j < 40; j++) {
				Slot slot = inventorySlots.getSlot(j);
				// isPointInRegion
				if (func_146978_c(slot.xDisplayPosition, slot.yDisplayPosition,
						16, 16, mouseX, mouseY)) {
					FontRenderer font = stack.getItem().getFontRenderer(stack);
					String itemName = currentTip.get(0);
					List<String> list = ImmutableList
							.of(stack.getRarity().rarityColor + itemName);
					drawHoveringText(list, mouseX, mouseY,
							font == null ? fontRendererObj : font);

					boolean leftSide = mouseX + 12
							+ fontRendererObj.getStringWidth(itemName) > width;
	...
	private void drawIngredientTooltip(int slotIndex, int mouseX, int mouseY,
			boolean leftSide) {

		WrappedRecipe recipe = null;

		int recipe_index = slotIndex + currentRowOffset * 8;
		if (recipe_index >= 0 && shownRecipes != null
				&& recipe_index < shownRecipes.size()) {
			WrappedRecipe r = shownRecipes.get(recipe_index);
			if (StackUtils.areCraftingEquivalent(r.getOutput(), inventorySlots
		...
		List<WrappedStack> ingredientList = canCraft
				&& !recipe.usedIngredients.isEmpty() ? StackUtils
				.collateStacks(recipe.usedIngredients) : recipe.collatedInputs;
		...
		if (ingredientList != null && !ingredientList.isEmpty()) {
			...
			// red: 0x50FF0000;// green: 0x5000A700;// vanilla purple:
			// 0x505000FF;
			int bgColor = 0xF0100010;
			int borderColor = canCraft ? 0x5000A700 : 0x50FF0000;
			int borderColorDark = (borderColor & 0xFEFEFE) >> 1 | borderColor
					& 0xFF000000;
			...
			drawGradientRect(xPos - 3, yPos - 4, xPos + width + 3, yPos - 3,
					bgColor, bgColor);
			drawGradientRect(xPos - 3, yPos + height + 3, xPos + width + 3,
					yPos + height + 4, bgColor, bgColor);
			drawGradientRect(xPos - 3, yPos - 3, xPos + width + 3, yPos
					+ height + 3, bgColor, bgColor);
			drawGradientRect(xPos - 4, yPos - 3, xPos - 3, yPos + height + 3,
					bgColor, bgColor);
			drawGradientRect(xPos + width + 3, yPos - 3, xPos + width + 4, yPos
					+ height + 3, bgColor, bgColor);

			drawGradientRect(xPos - 3, yPos - 3 + 1, xPos - 3 + 1, yPos
					+ height + 3 - 1, borderColor, borderColorDark);
			drawGradientRect(xPos + width + 2, yPos - 3 + 1, xPos + width + 3,
					yPos + height + 3 - 1, borderColor, borderColorDark);
			drawGradientRect(xPos - 3, yPos - 3, xPos + width + 3,
					yPos - 3 + 1, borderColor, borderColor);
			drawGradientRect(xPos - 3, yPos + height + 2, xPos + width + 3,
					yPos + height + 3, borderColorDark, borderColorDark);
	...
	private void renderItem(int xPos, int yPos, WrappedStack ws) {
		...
		if (is != null) {
			itemRender.renderItemAndEffectIntoGUI(fontRendererObj,
					mc.renderEngine, is, xPos, yPos);
			itemRender.renderItemOverlayIntoGUI(fontRendererObj,
					mc.renderEngine, is, xPos, yPos);
		} else {
			Ref.LOGGER
					.warn("Error rendering stack in recipe tooltip: is == null");
		}
	}
}