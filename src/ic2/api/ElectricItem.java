 ...
 * 
 * The charge or remaining capacity of an item can be determined by calling charge/discharge with ignoreTransferLimit and simulate set to true.
 */
public final class ElectricItem {
    ...
    public static int charge(ItemStack itemStack, int amount, int tier, boolean ignoreTransferLimit, boolean simulate) {
        try {
            if (ElectricItem_charge == null)
                ElectricItem_charge = Class.forName(getPackage() + ".common.ElectricItem").getMethod("charge", ItemStack.class, Integer.TYPE, Integer.TYPE, Boolean.TYPE, Boolean.TYPE);
    ...
    public static int discharge(ItemStack itemStack, int amount, int tier, boolean ignoreTransferLimit, boolean simulate) {
        try {
            if (ElectricItem_discharge == null)
                ElectricItem_discharge = Class.forName(getPackage() + ".common.ElectricItem").getMethod("discharge", ItemStack.class, Integer.TYPE, Integer.TYPE, Boolean.TYPE, Boolean.TYPE);
    ...
    /**
     * Determine if the specified electric item has at least a specific amount of EU. This is supposed to be used in the item code during operation,
     * for example if you want to implement your own electric item. BatPacks are not taken into account.
     * 
     * @param itemStack electric item's stack
    ...
    public static boolean canUse(ItemStack itemStack, int amount) {
        try {
            if (ElectricItem_canUse == null)
                ElectricItem_canUse = Class.forName(getPackage() + ".common.ElectricItem").getMethod("canUse", ItemStack.class, Integer.TYPE);
    ...
    /**
     * Try to retrieve a specific amount of energy from an Item, and if applicable, a BatPack. This is supposed to be used in the item code during
     * operation, for example if you want to implement your own electric item.
     * 
     * @param itemStack electric item's stack
    ...
    public static boolean use(ItemStack itemStack, int amount, EntityPlayer player) {
        try {
            if (ElectricItem_use == null)
                ElectricItem_use = Class.forName(getPackage() + ".common.ElectricItem").getMethod("use", ItemStack.class, Integer.TYPE, EntityPlayer.class);
    ...
    /**
     * Charge an item from the BatPack a player is wearing. This is supposed to be used in the item code during operation, for example if you want to
     * implement your own electric item. use() already contains this functionality.
     * 
     * @param itemStack electric item's stack
     * @param player player holding the item
     */
    public static void chargeFromArmor(ItemStack itemStack, EntityPlayer player) {
        try {
            if (ElectricItem_chargeFromArmor == null)
                ElectricItem_chargeFromArmor = Class.forName(getPackage() + ".common.ElectricItem").getMethod("chargeFromArmor", ItemStack.class, EntityPlayer.class);
